...
def tuxedo_line(key, value):
    return f"{key} = {value}"


def write_tuxedo_theme(path, model):
    colors = model["colors"]
    normal = dict(zip(TERMINAL_ROLE_NAMES, colors["ansi"]))
    dim = blend_colors(colors["background"], colors["foreground"], 0.55)
    panel = blend_colors(colors["background"], colors["foreground"], 0.08)
    border = blend_colors(colors["background"], colors["foreground"], 0.25)
    accent = normal["yellow"]
    values = {
        "name": model["name"] or "THEMaker Theme",
        "bg": format_hex(colors["background"]),
        "panel": format_hex(panel),
        "border": format_hex(border),
        "fg": format_hex(colors["foreground"]),
        "dim": format_hex(dim),
        "accent": format_hex(accent),
        "cursor": format_hex(colors["cursor"]),
        "selection": format_hex(colors["selection"]),
        "statusbar": format_hex(panel),
        "status_fg": format_hex(dim),
        "mode_fg": format_hex(colors["background"]),
        "mode_bg": format_hex(accent),
        "pri_a": format_hex(normal["red"]),
        "pri_b": format_hex(normal["yellow"]),
        "pri_c": format_hex(normal["green"]),
        "pri_d": format_hex(normal["blue"]),
        "pri_other": format_hex(normal["magenta"]),
        "project": format_hex(normal["green"]),
        "context": format_hex(normal["magenta"]),
        "due": format_hex(normal["yellow"]),
        "overdue": format_hex(normal["red"]),
        "today": format_hex(normal["red"]),
        "done": format_hex(dim),
        "selected": format_hex(colors["selection"]),
        "matched": format_hex(normal["cyan"]),
    }
    lines = [
        f"# Generated by {model['generator']} for tuxedo.",
        "# Copy into ${XDG_CONFIG_HOME:-$HOME/.config}/tuxedo/themes/ and press T to select it.",
    ]
    lines.extend(tuxedo_line(key, value) for key, value in values.items())
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
EXPORTERS = {
    ...
    "tuxedo": (".tuxedo.toml", write_tuxedo_theme),
...
def build_parser():
    ...
    parser.add_argument(
        "--format",
        dest="formats",
        help="Export formats: all, data, or any of iterm, terminal, kitty, alacritty, wezterm, coteditor, tuxedo, yaml, data.",
    )
    parser.add_argument(
        "--list-themes",
        action="store_true",
